...
@task(
    help={
        ...
        "persist": "Enable persistent data storage using redis as a database",
        "redis": "Redis server address (default is localhost)",
        "clean": "clean the database",
    }
)
def start(c, _open=False, debug=True, clean=False, persist=False, redis="localhost"):
    """Start the flask server"""
    if _open:
        webbrowser.open("http://localhost:5000")
    command = f"REDIS_ADDRESS={redis} "
    if persist:
        command += "PERSIST=yes "
    if clean:
    ...
    c.run(command + "FLASK_APP=python_url_shortener.app pipenv run flask run")
